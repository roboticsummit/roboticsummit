var grid = jQuery('#3596').masonry({
  itemSelector: '.scheduleday_wrapper',
  gutter: 40
});

jQuery('#3596 li .session_content_wrapper.expandable').on( 'click', function(e) {
...
jQuery('#session_filter_3596 li a').on( 'click', function(e) {
	var targetFilter = jQuery(this).attr('data-filter');
	
	jQuery('#session_filter_3596 li a').removeClass('active');
	jQuery(this).addClass('active');
	
	if(targetFilter != '')
	{
		jQuery('#3596 ul.scheduleday_wrapper li').removeClass('hide');
		
		jQuery('#3596 ul.scheduleday_wrapper li').each(function(){
...
jQuery('#session_expand_3596').on( 'click', function(e) {
	jQuery('#3596 ul.scheduleday_wrapper li .session_content_wrapper.expandable').trigger('click');
