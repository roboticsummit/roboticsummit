var grid = jQuery('#3487').masonry({
  itemSelector: '.scheduleday_wrapper',
  gutter: 40
});

jQuery('#3487 li .session_content_wrapper.expandable').on( 'click', function(e) {
...
jQuery('#session_filter_3487 li a').on( 'click', function(e) {
	var targetFilter = jQuery(this).attr('data-filter');
	
	jQuery('#session_filter_3487 li a').removeClass('active');
	jQuery(this).addClass('active');
	
	if(targetFilter != '')
	{
		jQuery('#3487 ul.scheduleday_wrapper li').removeClass('hide');
		
		jQuery('#3487 ul.scheduleday_wrapper li').each(function(){
...
jQuery('#session_expand_3487').on( 'click', function(e) {
	jQuery('#3487 ul.scheduleday_wrapper li .session_content_wrapper.expandable').trigger('click');
