var grid = jQuery('#3332').masonry({
  itemSelector: '.scheduleday_wrapper',
  gutter: 40
});

jQuery('#3332 li .session_content_wrapper.expandable').on( 'click', function(e) {
...
jQuery('#session_filter_3332 li a').on( 'click', function(e) {
	var targetFilter = jQuery(this).attr('data-filter');
	
	jQuery('#session_filter_3332 li a').removeClass('active');
	jQuery(this).addClass('active');
	
	if(targetFilter != '')
	{
		jQuery('#3332 ul.scheduleday_wrapper li').removeClass('hide');
		
		jQuery('#3332 ul.scheduleday_wrapper li').each(function(){
...
jQuery('#session_expand_3332').on( 'click', function(e) {
	jQuery('#3332 ul.scheduleday_wrapper li .session_content_wrapper.expandable').trigger('click');
