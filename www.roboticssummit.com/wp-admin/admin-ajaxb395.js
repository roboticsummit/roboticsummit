var grid = jQuery('#2845').masonry({
  itemSelector: '.scheduleday_wrapper',
  gutter: 40
});

jQuery('#2845 li .session_content_wrapper.expandable').on( 'click', function(e) {
...
jQuery('#session_filter_2845 li a').on( 'click', function(e) {
	var targetFilter = jQuery(this).attr('data-filter');
	
	jQuery('#session_filter_2845 li a').removeClass('active');
	jQuery(this).addClass('active');
	
	if(targetFilter != '')
	{
		jQuery('#2845 ul.scheduleday_wrapper li').removeClass('hide');
		
		jQuery('#2845 ul.scheduleday_wrapper li').each(function(){
...
jQuery('#session_expand_2845').on( 'click', function(e) {
	jQuery('#2845 ul.scheduleday_wrapper li .session_content_wrapper.expandable').trigger('click');
