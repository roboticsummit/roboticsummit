var grid = jQuery('#3723').masonry({
  itemSelector: '.scheduleday_wrapper',
  gutter: 40
});

jQuery('#3723 li .session_content_wrapper.expandable').on( 'click', function(e) {
...
jQuery('#session_filter_3723 li a').on( 'click', function(e) {
	var targetFilter = jQuery(this).attr('data-filter');
	
	jQuery('#session_filter_3723 li a').removeClass('active');
	jQuery(this).addClass('active');
	
	if(targetFilter != '')
	{
		jQuery('#3723 ul.scheduleday_wrapper li').removeClass('hide');
		
		jQuery('#3723 ul.scheduleday_wrapper li').each(function(){
...
jQuery('#session_expand_3723').on( 'click', function(e) {
	jQuery('#3723 ul.scheduleday_wrapper li .session_content_wrapper.expandable').trigger('click');
