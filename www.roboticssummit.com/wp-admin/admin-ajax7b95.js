var grid = jQuery('#3516').masonry({
  itemSelector: '.scheduleday_wrapper',
  gutter: 40
});

jQuery('#3516 li .session_content_wrapper.expandable').on( 'click', function(e) {
...
jQuery('#session_filter_3516 li a').on( 'click', function(e) {
	var targetFilter = jQuery(this).attr('data-filter');
	
	jQuery('#session_filter_3516 li a').removeClass('active');
	jQuery(this).addClass('active');
	
	if(targetFilter != '')
	{
		jQuery('#3516 ul.scheduleday_wrapper li').removeClass('hide');
		
		jQuery('#3516 ul.scheduleday_wrapper li').each(function(){
...
jQuery('#session_expand_3516').on( 'click', function(e) {
	jQuery('#3516 ul.scheduleday_wrapper li .session_content_wrapper.expandable').trigger('click');
