var grid = jQuery('#1753').masonry({
  itemSelector: '.scheduleday_wrapper',
  gutter: 40
});

jQuery('#1753 li .session_content_wrapper.expandable').on( 'click', function(e) {
...
jQuery('#session_filter_1753 li a').on( 'click', function(e) {
	var targetFilter = jQuery(this).attr('data-filter');
	
	jQuery('#session_filter_1753 li a').removeClass('active');
	jQuery(this).addClass('active');
	
	if(targetFilter != '')
	{
		jQuery('#1753 ul.scheduleday_wrapper li').removeClass('hide');
		
		jQuery('#1753 ul.scheduleday_wrapper li').each(function(){
...
jQuery('#session_expand_1753').on( 'click', function(e) {
	jQuery('#1753 ul.scheduleday_wrapper li .session_content_wrapper.expandable').trigger('click');
