var grid = jQuery('#1433').masonry({
  itemSelector: '.scheduleday_wrapper',
  gutter: 40
});

jQuery('#1433 li .session_content_wrapper.expandable').on( 'click', function(e) {
...
jQuery('#session_filter_1433 li a').on( 'click', function(e) {
	var targetFilter = jQuery(this).attr('data-filter');
	
	jQuery('#session_filter_1433 li a').removeClass('active');
	jQuery(this).addClass('active');
	
	if(targetFilter != '')
	{
		jQuery('#1433 ul.scheduleday_wrapper li').removeClass('hide');
		
		jQuery('#1433 ul.scheduleday_wrapper li').each(function(){
...
jQuery('#session_expand_1433').on( 'click', function(e) {
	jQuery('#1433 ul.scheduleday_wrapper li .session_content_wrapper.expandable').trigger('click');
